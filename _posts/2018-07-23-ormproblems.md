...
This and more complicated queries like this, don't work for me. At least, it's never worked when I've tried. Next option is to have the strongly-typed objects we want as a separate Domain Model, and then have a second Data Model that we need to maintain and then we need a mapping layer to go from one to the other. We've doubled the amount of work we need to do, all because the ORM doesn't understand that sometimes we need different formats for different purposes.
